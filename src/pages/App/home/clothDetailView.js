...
function ClothDetail({route, navigation}) {
  const {image, name, pricePermeter, id, description, material, color} =
    route.params;
      ...
      <View style={{padding: 20}}>
        <View style={[styles.boxWithShadow, styles.counter]}>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <Pressable
              ...
              onPress={() => setQuantity(selectedQuantity - 1)}>
              <Ionicons name="remove" color="#E8875C" size={32} />
            </Pressable>
            <Text style={{fontSize: 16, color: '#324755'}}>
              {selectedQuantity} mtr
            </Text>
            <Pressable
              style={styles.circle}
              onPress={() => setQuantity(selectedQuantity + 1)}>
          ...
          <View style={{flexDirection: 'row', alignItems: 'baseline'}}>
            <Text style={{marginLeft: 5, fontSize: 14, color: '#7D8184'}}>
              Rs.
            </Text>
            <Text style={{color: '#E8875C', fontSize: 20, fontWeight: '500'}}>
              {selectedQuantity * pricePermeter}
            </Text>
          </View>
        </View>
        ...
        <View style={{marginTop: 20}}>
          ...
          <Text style={styles.sectionContent}>
            Material Type: {material?.toLowerCase()}
          </Text>
          <Text style={styles.sectionContent}>
