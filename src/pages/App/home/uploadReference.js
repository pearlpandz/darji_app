...
function UploadReference({ orderData }) {
    ...
    const handleImgUpload = async (_imageList) => {
        try {
            ...
            await Promise.all(_imageList.map((item, index) => {
        ...
        <ScrollView style={{ color: '#fff' }}>
            <View style={styles.imageContainer}>
                        ...
                        <View style={{ overflow: 'hidden', borderRadius: 20 }}>
                            <FlatList
                                ...
                                renderItem={({ item, index }) => {
                                    return (
                                        <View key={item.index} style={{ marginRight: 2, position: 'relative' }}>
                                            <Pressable onPress={() => removeItem(item)} style={{
                                                width: '100%',
                                                flexDirection: 'row',
                                                alignItems: 'center',
                                                ...
                                                <Text style={{ color: '#fff', textTransform: 'capitalize' }}>remove</Text>
                ...
                <View style={{ paddingHorizontal: 20 }}>
                    ...
                    >
                        <Text style={{ textAlign: 'center', color: '#fff', padding: 12, fontSize: 14, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 0.8 }}>
